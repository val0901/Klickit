...
class OrdersModel extends \W\Model\Model 
{
	...
	/*Requête sur la table orders avec jointure sur la table user pour récupérer toute les commandes "Fini"*/
	public function findAllOrdersEnd($page, $max)
	{
		//on definit la page de démarrage
		$debut = ($page - 1) * $max;

		// requête avec jointure où on definit les variables de la page de démarrage($debut) et là le nombre de lignes par page($max)
		$sql = 'SELECT ' .$this->table.'.*, u.firstname, u.lastname FROM ' . $this->table . ' LEFT JOIN user AS u ON '.$this->table.'.idMember = u.id WHERE '.$this->table.'.order_process = "Fini" ORDER BY ' .$this->table.'.date_creation DESC LIMIT :debut, :max';
